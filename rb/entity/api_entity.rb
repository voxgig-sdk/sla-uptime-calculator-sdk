# SlaUptimeCalculator SDK Api entity
...
class ApiEntity
  ...
  def data_set(args)
    if args
      @_data = SlaUptimeCalculatorHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = SlaUptimeCalculatorHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Api, Hash] the loaded Api; raises SlaUptimeCalculatorError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = SlaUptimeCalculatorHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
