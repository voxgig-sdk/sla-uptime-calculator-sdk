...
require_relative 'SlaUptimeCalculator_types'
...
class SlaUptimeCalculatorSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = SlaUptimeCalculatorHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = SlaUptimeCalculatorHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, SlaUptimeCalculatorFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    SlaUptimeCalculatorUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = SlaUptimeCalculatorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = SlaUptimeCalculatorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = SlaUptimeCalculatorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => SlaUptimeCalculatorError.new(
        "#{op}_allow",
        "SlaUptimeCalculatorSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue SlaUptimeCalculatorError => err
    ...
    if fetched.is_a?(Hash)
      status = SlaUptimeCalculatorHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = SlaUptimeCalculatorError.new(
        "graphql_error", "SlaUptimeCalculatorSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = SlaUptimeCalculatorSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
