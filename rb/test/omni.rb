...
module SlaUptimeCalculatorOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if SlaUptimeCalculatorOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = SlaUptimeCalculatorOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| SlaUptimeCalculatorOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || SlaUptimeCalculatorOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = SlaUptimeCalculatorSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = SlaUptimeCalculatorError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { SlaUptimeCalculatorOmni.errify(err) }
