...
// SlaUptimeCalculator SDK feature test
//
...
class FtCtrl extends SlaUptimeCalculatorControl
{
...
class FtClient
{
    ...
    private SlaUptimeCalculatorUtility $utility;

    public function __construct(SlaUptimeCalculatorUtility $utility, array $options)
    {
    ...
    public function get_utility(): SlaUptimeCalculatorUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SlaUptimeCalculatorContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public SlaUptimeCalculatorContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (SlaUptimeCalculatorContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SlaUptimeCalculatorConfig::make_feature($name);
    ...
    private static function buildUrl(SlaUptimeCalculatorSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SlaUptimeCalculatorError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SlaUptimeCalculatorResponse($fetched);
        }
    ...
    private function fail(SlaUptimeCalculatorContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SlaUptimeCalculatorContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
