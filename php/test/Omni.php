...
//    only - a typed SlaUptimeCalculatorContext there would make every ctx
...
final class SlaUptimeCalculatorOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new SlaUptimeCalculatorContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new SlaUptimeCalculatorSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new SlaUptimeCalculatorResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new SlaUptimeCalculatorError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new SlaUptimeCalculatorResponse($clean['response']);
    ...
    public static function err_from_map($m): ?SlaUptimeCalculatorError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new SlaUptimeCalculatorError(is_string($code) ? $code : '', $msg);
    }
