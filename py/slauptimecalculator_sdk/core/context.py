...
from slauptimecalculator_sdk.core.control import SlaUptimeCalculatorControl
...
from slauptimecalculator_sdk.core.helpers import get_ctx_prop, to_map
...
class SlaUptimeCalculatorContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = SlaUptimeCalculatorControl()
        ...
        if isinstance(sp, SlaUptimeCalculatorSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, SlaUptimeCalculatorResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, SlaUptimeCalculatorResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return SlaUptimeCalculatorOperation({})
        ...
        op = SlaUptimeCalculatorOperation({
            "entity": entname,
            "name": opname,
            "input": inpt,
            "points": points,
    ...
    def make_error(self, code, msg):
        return SlaUptimeCalculatorError(code, msg, self)
