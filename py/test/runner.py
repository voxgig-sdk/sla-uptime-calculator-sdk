# SlaUptimeCalculator SDK test runner
...
class SlaUptimeCalculatorTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            SlaUptimeCalculatorTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = SlaUptimeCalculatorTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = SlaUptimeCalculatorTestRunner.getenv("SLA_UPTIME_CALCULATOR_TEST_LIVE")
        override = SlaUptimeCalculatorTestRunner.getenv("SLA_UPTIME_CALCULATOR_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = SlaUptimeCalculatorTestRunner.getenv(key)
        ...
        explain = SlaUptimeCalculatorTestRunner.getenv("SLA_UPTIME_CALCULATOR_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if SlaUptimeCalculatorTestRunner._test_control is not None:
            return SlaUptimeCalculatorTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                SlaUptimeCalculatorTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            SlaUptimeCalculatorTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = SlaUptimeCalculatorTestRunner.load_test_control()
...
def load_env_local():
    SlaUptimeCalculatorTestRunner.load_env_local()


def env_override(m):
    return SlaUptimeCalculatorTestRunner.env_override(m)


def entity_data(v):
    return SlaUptimeCalculatorTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return SlaUptimeCalculatorTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return SlaUptimeCalculatorTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return SlaUptimeCalculatorTestRunner.load_test_control()


def live_client_options():
    return SlaUptimeCalculatorTestRunner.live_client_options()


def live_delay_ms():
    return SlaUptimeCalculatorTestRunner.live_delay_ms()
