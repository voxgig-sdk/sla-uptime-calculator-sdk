# sla-uptime-calculator-cli

boru-driven command-line client **and** interactive REPL for the SlaUptimeCalculator
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/sla-uptime-calculator-cli)
make build

# 2. See usage (words, entities, env vars)
./sla-uptime-calculator-cli --help

# 3. Provide credentials once, via the environment
export SLA_UPTIME_CALCULATOR_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./sla-uptime-calculator-cli load 1 api            # {id:1} shorthand
./sla-uptime-calculator-cli load '{id:1}' api       # explicit match map

# 5. Override the API base URL for a single call
SLA_UPTIME_CALCULATOR_BASE=https://api.example.com ./sla-uptime-calculator-cli load 1 api

# 6. No arguments -> interactive REPL
./sla-uptime-calculator-cli
sla-uptime-calculator> load 1 api
sla-uptime-calculator> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/sla-uptime-calculator-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/sla-uptime-calculator-cli load 1 api
   ```
...
```sh
./sla-uptime-calculator-cli load 1 api          # scalar shorthand for {id:1}
./sla-uptime-calculator-cli load '{id:1}' api     # explicit match map
```
...
./sla-uptime-calculator-cli load 1 api
```
...
Run with no arguments to open a REPL (prompt `sla-uptime-calculator>`). Each line is
evaluated as its own boru expression:

```text
$ ./sla-uptime-calculator-cli
sla-uptime-calculator> load 1 api
sla-uptime-calculator> /help
sla-uptime-calculator> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/sla-uptime-calculator-cli`. |
...
(`./sla-uptime-calculator-cli <expr>`) and interactively (the REPL), and expressions compose the
