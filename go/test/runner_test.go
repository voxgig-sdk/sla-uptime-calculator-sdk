...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("SLA_UPTIME_CALCULATOR_TEST_LIVE") == "TRUE" ||
		os.Getenv("SLA_UPTIME_CALCULATOR_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("SLA_UPTIME_CALCULATOR_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.SlaUptimeCalculatorSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.SlaUptimeCalculatorSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.SlaUptimeCalculatorError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.SlaUptimeCalculatorSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.SlaUptimeCalculatorError{Msg: msg, Code: code}
}
