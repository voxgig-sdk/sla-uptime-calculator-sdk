...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("SLAUPTIMECALCULATOR_TEST_LIVE") == "TRUE" ||
		os.Getenv("SLAUPTIMECALCULATOR_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("SLAUPTIMECALCULATOR_TEST_EXPLAIN"); explain != "" {
