...
import (
	...
	omni "github.com/voxgig-sdk/sla-uptime-calculator-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.SlaUptimeCalculatorError); is {
				out := map[string]any{
					"name":    "SlaUptimeCalculatorError",
...
func omniCtx(arg any, client *sdk.SlaUptimeCalculatorSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.SlaUptimeCalculatorSDK); is {
			client = live
