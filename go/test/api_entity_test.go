...
func TestApiEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set SLA_UPTIME_CALCULATOR_TEST_API_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func apiBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("SLA_UPTIME_CALCULATOR_TEST_API_ENTID")
	...
	idmapResolved := core.ToMapAny(env["SLA_UPTIME_CALCULATOR_TEST_API_ENTID"])
	...
	if env["SLA_UPTIME_CALCULATOR_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewSlaUptimeCalculatorSDK(core.ToMapAny(mergedOpts))
	}
