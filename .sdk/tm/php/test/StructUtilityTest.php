...
// SlaUptimeCalculator SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = SlaUptimeCalculatorOmni::makeStructRunner(
    ...
    {
        return SlaUptimeCalculatorOmni::objmodel(SlaUptimeCalculatorOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return SlaUptimeCalculatorOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return SlaUptimeCalculatorOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(SlaUptimeCalculatorOmni::structfix($now, Struct::undef()))
                        !== json_encode(SlaUptimeCalculatorOmni::structfix($val, Struct::undef()))) {
