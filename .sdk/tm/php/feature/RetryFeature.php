...
// SlaUptimeCalculator SDK retry feature
...
class SlaUptimeCalculatorRetryFeature extends SlaUptimeCalculatorBaseFeature
{
    ...
    public function init(SlaUptimeCalculatorContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (SlaUptimeCalculatorContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(SlaUptimeCalculatorContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(SlaUptimeCalculatorContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
