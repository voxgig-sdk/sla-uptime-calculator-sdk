...
module SlaUptimeCalculatorUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = SlaUptimeCalculatorResponse.new({})
    ...
    if fetch_err
      ...
      response = SlaUptimeCalculatorResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = SlaUptimeCalculatorResponse.new(fetched)
    else
