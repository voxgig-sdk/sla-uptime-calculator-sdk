...
//     (\`new SlaUptimeCalculatorSDK(...)\` / \`SlaUptimeCalculatorSDK.test(...)\` become
//     \`SlaUptimeCalculatorSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'SlaUptimeCalculatorSDK'
...
// rewrite real client construction (\`new SlaUptimeCalculatorSDK(...)\` and any
// \`SlaUptimeCalculatorSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+SlaUptimeCalculatorSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/SlaUptimeCalculatorSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
