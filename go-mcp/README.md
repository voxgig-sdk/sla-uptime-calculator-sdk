# sla-uptime-calculator-mcp

[MCP](https://modelcontextprotocol.io) server exposing the SlaUptimeCalculator SDK as
two agent tools — `sla-uptime-calculator_list` and `sla-uptime-calculator_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/sla-uptime-calculator-mcp)
make build

# 2. Provide credentials via the environment
export SLA_UPTIME_CALCULATOR_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user sla-uptime-calculator \
  -- /absolute/path/to/sla-uptime-calculator-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./sla-uptime-calculator-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// sla-uptime-calculator_list: first page of records
{ "entity": "api" }
{ "entity": "api", "query": { } }

// sla-uptime-calculator_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/sla-uptime-calculator-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user sla-uptime-calculator \
     -- "$PWD"/dist/*/sla-uptime-calculator-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `sla-uptime-calculator_list` and `sla-uptime-calculator_load` tools now appear
   in new sessions. Ask the agent to *"list api using sla-uptime-calculator"*
   and it calls `sla-uptime-calculator_list` with `{"entity":"api"}`.
...
```sh
export SLA_UPTIME_CALCULATOR_APIKEY=sk_live_xxx            # API key
export SLA_UPTIME_CALCULATOR_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `sla-uptime-calculator_list` tool
...
### Call the `sla-uptime-calculator_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `sla-uptime-calculator_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `sla-uptime-calculator_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SLA_UPTIME_CALCULATOR_APIKEY` | API key sent with every request. |
| `SLA_UPTIME_CALCULATOR_BASE` | Optional override of the API base URL. |
...
```sh
./sla-uptime-calculator-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"sla-uptime-calculator_load","arguments":{"entity":"api","query":{"id":1}}}}'
```
